...
( function(factory) {
        ...
        _.extend(Backbone.ModelBinder.prototype, {
            ...
            bind : function(model, rootEl, attributeBindings, modelSetOptions) {
                Ti.API.debug("In ModelBinder bind "+JSON.stringify(model));
                this.unbind();
                ...
                if (!this._rootEl) {
                	Titanium.API.info('model : ' + JSON.stringify(model));
                	Titanium.API.info('rootEl : ' + JSON.stringify(rootEl));
                	Titanium.API.info('attributeBindings : ' + JSON.stringify(attributeBindings));
                	Titanium.API.info('modelSetOptions : ' + JSON.stringify(modelSetOptions));
            ...
            _initializeAttributeBindings : function() {
                Ti.API.debug('At the beginning of _initializeAttributeBindings ' + JSON.stringify(this._attributeBindings));
...
// 
                    // for ( bindingCount = 0; bindingCount < attributeBinding.elementBindings.length; bindingCount++) {
                        ...
                        // this._bindElementToModel(elementBinding.el);
            ...
            _bindViewToModel : function() {
             //   $(this._rootEl).delegate('', 'change', this._onElChanged);
               ...
               var bindingKey, attributeBinding, bindingCount, elementBinding;
                    for (bindingKey in this._attributeBindings) {
                        attributeBinding = this._attributeBindings[bindingKey];
                        for ( bindingCount = 0; bindingCount < attributeBinding.elementBindings.length; bindingCount++) {
                            elementBinding = attributeBinding.elementBindings[bindingCount];
            ...
            _bindElementToModel : function(element) {
                Ti.API.info('Bind element to model: ' + JSON.stringify(element));
                element.addEventListener('change', this._onElChanged);
            },

            _unbindViewToModel : function(element) {
                // if (this._rootEl) {
                ...
                var bindingKey, attributeBinding, bindingCount, elementBinding;
                    for (bindingKey in this._attributeBindings) {
                        ...
                        for ( bindingCount = 0; bindingCount < attributeBinding.elementBindings.length; bindingCount++) {
                            elementBinding = attributeBinding.elementBindings[bindingCount];
                            this._unbindElementToModel(elementBinding.el);
                        }
                    }
            },

            _unbindElementToModel : function(element) {
                element.removeEventListener('change', this._onElChanged);
            },

            _onElChanged : function(event) {
                Ti.API.info('_onElChanged: ' + JSON.stringify(event));
                var el, elBindings, elBindingCount, elBinding;

                // el = $(event.target)[0];
                el = event.source;
                elBindings = this._getElBindings(el);
                Ti.API.info('on el changed: ' + elBindings.length);
            ...
            // No need for that function since only editable element can trigger change event
            ...
            _copyModelToView : function(attributeBinding) {
                ...
                for ( elementBindingCount = 0; elementBindingCount < attributeBinding.elementBindings.length; elementBindingCount++) {
                    ...
                    if (!elementBinding.isSetting) {
                        ...
                        // }
                        elementBinding.el[elementBinding.attribute] = convertedValue;
                    }
                }
            },
            ...
            _setModel : function(elementBinding/*, el*/) {
                ...
                var elVal = elementBinding.el[elementBinding.attribute];
                Ti.API.info('elVal: ' + elVal + ' - ' + JSON.stringify(elementBinding.el));
                ...
                var opts = _.extend({}, this._modelSetOptions, {
                    changeSource : 'ModelBinder'
                });
                //  this._model.set(data, opts);
                this._model.set(elementBinding.attributeBinding.attributeName, elVal);
                Ti.API.info('setModel: ' + elVal + ' - ' + JSON.stringify(this._model));
            },
